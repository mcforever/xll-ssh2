...
function meizzDayClick(n,ex)  //点击显示框选取日期，主输入函数*************
{
  ...
  if (outObject)
  {
	...
	if(outID=='openAccountDate'){
	  WriteDateToHuaxin(yy,mm,n,hh,mi);
	}else{
...
function WriteDateToHuaxin(yy,mm,n,hh,mi)
{
	if (bUseTime)
	{
	...
	else
	{
	   ...
	   if(outID='openAccountDate'){
	       var gdate = yy + "/" + format(mm) + "/" + format(n);
	       var d = new   Date(new   Date(gdate).getTime()+4*24*60*60*1000);
